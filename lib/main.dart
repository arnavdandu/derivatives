...
List d() {
  ...
  String derivativeTex = r"<p> $$" +
      r"{d \over dx}" +
      "{(${a}x+$b)^$c$dm(${d}x+$e)^$f}" +
      r"$$</p>";
  //String derivative = "derivative+of+($a(x)%2b$b)^$c+/+($d(x)%2b$e)^$f";

  String derivative;
  (g == 0)
      ? derivative = "($a(x)%2b$b)^$c / ($d(x)%2b$e)^$f"
      : derivative = "($a(x)%2b$b)^$c($d(x)%2b$e)^$f";

  List derivatives = [derivativeTex, derivative];
  return derivatives;
}
*/
/*
class StartPage extends StatelessWidget {
  StartPage({this.derivatives});
  List derivatives;
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Column(
          children: [
            Text("Start practicing derivatives.",
                style: TextStyle(
                    fontSize:
                        (MediaQuery.of(context).size.width > 1300) ? 40 : 30,
                    fontWeight: FontWeight.bold),
                textAlign: TextAlign.center),
            SizedBox(height: 100),
            CustomRaisedButton(
                width: (MediaQuery.of(context).size.width > 1300) ? 400 : 200,
                height: (MediaQuery.of(context).size.width > 1300) ? 100 : 50,
                child: Text(
                  "Begin",
